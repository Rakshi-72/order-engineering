 ...
 *
 * <p>Shared state (id, status, price) and the status-transition guard live here.
 * Subclasses only add the fields that make them unique.</p>
 *
 * <p>equals / hashCode are on {@code itemId} only — this is <b>entity</b> equality,
 * not value equality.  Two items with the same base fields but different types
 * must never compare as equal.</p>
 */
public abstract class OrderItem {
    ...
    public boolean isActive() { return status == ItemStatus.ACTIVE; }
    ...
    protected static UUID newItemId() { return UUID.randomUUID(); }
}
